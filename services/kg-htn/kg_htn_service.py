...
app = FastAPI(
    title="Hypertension Diet KG Service",
    description="Neo4j-backed KG for Hypertension (DASH Diet) constraints",
    version="1.0.0",
)

class FoodCheckRequest(BaseModel):
    condition: str = "Hypertension"  # Default changed to Hypertension
    foods: List[str]
...
# -------------------------------------------------------------------
# Core Logic (Identical to Diabetes, but queried with dynamic Condition)
# -------------------------------------------------------------------
def classify_food(tx, condition: str, food: str):
    ...
    if rec is None:
        return {"food": food, "status": "UNKNOWN", "reasons": ["Food not found in KG"]}
    ...
    if avoid_rel:
        ...
        status = "UNKNOWN"
        reasons = [f"No explicit relation for {condition}"]

    return {"food": food, "status": status, "reasons": reasons}

@app.post("/v1/hypertension/kg/check_foods", response_model=FoodCheckResponse)
