#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Canonical Diabetes LoRA training script for Gaudi (kept as train.py for consistency)
import os, json
from pathlib import Path
from typing import Dict, List, Any

# ---------------------- Args from environment ----------------------
# Keep the same Qwen2.5 base model
...
TRAIN_FILE = os.getenv(
    "DIAB_TRAIN_JSONL",
    "/workspace/data/diabetes/curated/train.jsonl",
)

EVAL_FILE = os.getenv(
    "DIAB_EVAL_JSONL",
    "/workspace/data/diabetes/curated/val.jsonl",
)

OUT_DIR = os.getenv(
    "DIAB_OUT_DIR",
    "/workspace/models/diabetes_qwen_lora",
)

MAX_LEN = int(os.getenv("DIAB_MAX_LEN", "2048"))
EPOCHS  = float(os.getenv("DIAB_EPOCHS", "2"))
BSZ     = int(os.getenv("DIAB_BSZ", "4"))
GACC    = int(os.getenv("DIAB_GACC", "8"))

GAUDI_CFG_DIR = os.getenv("GAUDI_CFG_DIR", "/workspace/gaudi2_cfg")


# ---------------------- Basic assertions ---------------------------
assert Path(BASE_MODEL, "config.json").is_file(), f"❌ Model path invalid: {BASE_MODEL}"
assert Path(TRAIN_FILE).is_file(),                 f"❌ Diabetics train jsonl missing: {TRAIN_FILE}"
if EVAL_FILE:
    assert Path(EVAL_FILE).is_file(),              f"❌ Diabetics eval jsonl missing: {EVAL_FILE}"
...
ds_tr = ds_train.map(tokenize_fn, batched=True, remove_columns=remove_cols)
ds_ev = None
if ds_eval:
    ds_ev = ds_eval.map(
...
print("✅ Diabetes training done. Adapters saved to:", OUT_DIR)
