# Canonical HTN LoRA training script for Gaudi (kept as train.py for consistency)
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
# Point to hypertension JSONL we just created
TRAIN_FILE = os.getenv(
    "HTN_TRAIN_JSONL",
    "/workspace/data/hypertension/curated/train.jsonl",
)
EVAL_FILE = os.getenv(
    "HTN_EVAL_JSONL",
    "/workspace/data/hypertension/curated/val.jsonl",
)  # optional but we have it

OUT_DIR = os.getenv(
    "HTN_OUT_DIR",
    "/workspace/models/hypertension_qwen_lora",
)
...
assert Path(TRAIN_FILE).is_file(),                 f"❌ HTN train jsonl missing: {TRAIN_FILE}"
if EVAL_FILE:
    assert Path(EVAL_FILE).is_file(),              f"❌ HTN eval jsonl missing: {EVAL_FILE}"
...
print("✅ HTN training done. Adapters saved to:", OUT_DIR)
