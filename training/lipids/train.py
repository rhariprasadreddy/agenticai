...
TRAIN_FILE = os.getenv(
    "HTN_TRAIN_JSONL",
    "/workspace/data/lipids/curated/train.jsonl",
)
EVAL_FILE = os.getenv(
    "HTN_EVAL_JSONL",
    "/workspace/data/lipids/curated/val.jsonl",
...
assert Path(TRAIN_FILE).is_file(), \
    f"❌ Lipids train jsonl missing: {TRAIN_FILE}"
if EVAL_FILE:
    assert Path(EVAL_FILE).is_file(), \
        f"❌ Lipids eval jsonl missing: {EVAL_FILE}"
...
print("✅ Lipids training done. Adapters saved to:", OUT_DIR)
