...
MERGED = Path("/workspace/models/hypertension_qwen_merged_fp16")
PROMPTS = Path("/workspace/eval/prompts_hypertension.txt")
OUT = Path("/workspace/eval/gaudi_htn_outputs.jsonl")
    ...
    with OUT.open("w") as out_f, torch.no_grad():
        for idx, prompt in enumerate(prompts):
            ...
            rec = {
                ...
                "model": "gaudi_htn_qwen_fp16",
            }
            out_f.write(json.dumps(rec, ensure_ascii=False) + "\n")

    print(f"\n✅ Saved Gaudi HTN outputs to: {OUT}")
